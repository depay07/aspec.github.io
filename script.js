...
function calculateAreaSpeed() {
    ...
    if (cameraFPS > 0) {
        if (cameraFPS >= requiredFPS) {
            fpsComparison = `<p class="good">선택한 카메라 (${cameraFPS} FPS)는 요구 사양 (${requiredFPS.toFixed(2)} FPS)을 만족합니다. (OK)</p>`;
        } else {
            fpsComparison = `<p class="bad">선택한 카메라 (${cameraFPS} FPS)가 요구 사양 (${requiredFPS.toFixed(2)} FPS)보다 낮습니다! (NG)</p>`;
        }
    } else {
